...
def cert_validexts(cert, policy=[]):
    ...
    for check in policy:
        ...
        if not check[1](ext):
            raise x509.verification.VerificationError("Certificate extensions does not match expected va")
...
def valida_certALICE(ca_cert):
    try:
        ...
        #cert_validexts(cert, [(x509.ExtensionOID.EXTENDED_KEY_USAGE, lambda e: x509.oid.ExtendedKeyUsag
